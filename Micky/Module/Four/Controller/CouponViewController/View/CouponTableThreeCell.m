...
@implementation CouponTableThreeCell
@synthesize BGView;
@synthesize leftView;
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        ...
        right_imgV.image = [UIImage imageNamed:@"user_ expired"];
...
- (void)setCouponExpiredModel:(CouponModel *)couponExpiredModel {
    ...
    label_3.text = [Gobal isEmptyString:couponExpiredModel.name]?@"--":couponExpiredModel.name;
    label_4.text = [Gobal isEmptyString:couponExpiredModel.startDate]?@"开始时间：--": [NSString stringWithFormat:@"开始时间：%@",couponExpiredModel.startDate];
    label_5.text = [Gobal isEmptyString:couponExpiredModel.endDate]?@"结束时间：--": [NSString stringWithFormat:@"结束时间：%@",couponExpiredModel.endDate];
