...
@implementation CouponTableOneCell
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        ...
        leftView.backgroundColor = BUTTON_COLOR_NORMA;
...
- (void)setCouponNotUserModel:(CouponModel *)couponNotUserModel {
    ...
    label_3.text = [Gobal isEmptyString:couponNotUserModel.name]?@"--":couponNotUserModel.name;
    label_4.text = [Gobal isEmptyString:couponNotUserModel.startDate]?@"开始时间：--": [NSString stringWithFormat:@"开始时间：%@",couponNotUserModel.startDate];
    label_5.text = [Gobal isEmptyString:couponNotUserModel.endDate]?@"结束时间：--": [NSString stringWithFormat:@"结束时间：%@",couponNotUserModel.endDate];
}
