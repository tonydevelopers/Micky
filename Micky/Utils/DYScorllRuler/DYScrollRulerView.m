...
-(UITextField *)valueTF{
    if (!_valueTF) {
        ...
        _valueTF.backgroundColor          = [UIColor clearColor];
    }
    return _valueTF;
}
-(UILabel *)unitLab{
    if (!_unitLab) {
        ...
        _unitLab.textColor = [UIColor whiteColor];
    }
    return _unitLab;
}
#pragma mark -- 设置标尺底层颜色
...
//    [_collectionView setContentOffset:CGPointMake((int)realValue*RulerGap, 0) animated:animated];
    ...
    int X = (int)realValue/10;
    int Y = (int)realValue%10;
    
    if (Y >=5) {
        [_collectionView setContentOffset:CGPointMake((X+1)*RulerGap*10, 0) animated:animated];
    }else {
        [_collectionView setContentOffset:CGPointMake(X*RulerGap*10, 0) animated:animated];
    }
...
//        }RGBColor(28, 180, 254)
//        rulerView.backgroundColor        =   [UIColor colorWithRed:235.0/255.0 green:65.0/255.0 blue:32.0/255.0 alpha:1.0];
        rulerView.backgroundColor        =   [UIColor colorWithRed:89.0/255.0 green:141.0/255.0 blue:229.0/255.0 alpha:1.0];
//        rulerView.minValue = _step*(indexPath.item-1)*_betweenNum;
//        rulerView.maxValue = _step*indexPath.item*_betweenNum;
        rulerView.minValue = _minValue+_step*(indexPath.item-1)*_betweenNum;
        rulerView.maxValue = _maxValue+_step*indexPath.item*_betweenNum;
...
//    }
    ...
    int X = value/10;
    if (Y>=5) {
        totalV = (X+1)*100 +_minValue;
    }else {
        totalV = X*100 +_minValue;
    }
    ...
    if (_scrollByHand) {
        if (totalValue >= _maxValue) {
            ...
            //            _valueTF.text = [NSString stringWithFormat:@"%.1f",value*_step +_minValue];
            _valueTF.text = [NSString stringWithFormat:@"%.1f",totalV];
        }
    }
    if (self.delegate && [self.delegate respondsToSelector:@selector(dyScrollRulerView:valueChange:)]) {
        //        [self.delegate scrollRulerView:self changeWithValue:totalValue];
        [self.delegate dyScrollRulerView:self valueChange:totalV];
    }
