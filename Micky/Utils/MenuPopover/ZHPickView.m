...
//  ZHpickView
//
//  Created by ZN on 14-11-18.
//  Copyright (c) 2014年 张宁. All rights reserved.
//
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _showView.frame = CGRectMake( 0, 1000, [UIScreen mainScreen].bounds.size.width, 0);
        _showView.backgroundColor = [UIColor colorWithWhite:0.2 alpha:0.5];
        [[UIApplication sharedApplication].keyWindow addSubview:_showView];
        _showView.userInteractionEnabled = YES;
        _showView.alpha = 0;
        _tap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(remove)];
        [_showView addGestureRecognizer:_tap];
...
-(void)setUpPickView{
    ...
    [self addSubview:[Gobal drawSolidLinePointX:pickView.frame.size.width/2.0-0.5 PointY:0 lineW:1 lineH:40 lineColor:@[@"220",@"220",@"220"] lineAlpha:1.0]];
    [self addSubview:[Gobal drawSolidLinePointX:0 PointY:39 lineW:pickView.frame.size.width  lineH:1 lineColor:@[@"220",@"220",@"220"] lineAlpha:1.0]];

    
//    [self addSubview:[Gobal autolineFrameWidth:pickView.frame.size.width/2.0-0.5 height:0 lineW:1 lineH:40 lineAlpha:0.5]];
//    [self addSubview:[Gobal autolineFrameWidth:0 height:39 lineW:pickView.frame.size.width lineH:1 lineAlpha:0.5]];
}
...
-(void)setUpDatePickerWithdatePickerMode:(UIDatePickerMode)datePickerMode{
    ...
    [self addSubview:[Gobal drawSolidLinePointX:datePicker.frame.size.width/2.0-0.5 PointY:0 lineW:1 lineH:40 lineColor:@[@"220",@"220",@"220"] lineAlpha:1.0]];
    [self addSubview:[Gobal drawSolidLinePointX:0 PointY:39 lineW:datePicker.frame.size.width  lineH:1 lineColor:@[@"220",@"220",@"220"] lineAlpha:1.0]];
    
    
//    [self addSubview:[Gobal autolineFrameWidth:datePicker.frame.size.width/2.0-0.5 height:0 lineW:1 lineH:40 lineAlpha:0.5]];
//    [self addSubview:[Gobal autolineFrameWidth:0 height:39 lineW:datePicker.frame.size.width lineH:1 lineAlpha:0.5]];
...
-(UIToolbar *)setToolbarStyle{
    ...
    buttonL.frame = CGRectMake(0,0,[UIScreen mainScreen].bounds.size.width/2.0-1,40);
    [buttonL setTitle:@"取消" forState:UIControlStateNormal];
    [buttonL setTitleColor:[UIColor colorWithRed:105/255.0 green:124/255.0 blue:214/255.0 alpha:1.0] forState:UIControlStateNormal];
    [buttonL addTarget:self action:@selector(remove) forControlEvents:UIControlEventTouchUpInside];
    UIBarButtonItem *lefttem = [[UIBarButtonItem alloc] initWithCustomView:buttonL];
  
    UIButton *buttonR = [UIButton buttonWithType:UIButtonTypeCustom];
    buttonR.frame = CGRectMake(0,0,[UIScreen mainScreen].bounds.size.width/2.0-1,40);
    [buttonR setTitle:@"确定" forState:UIControlStateNormal];
    [buttonR setTitleColor:[UIColor colorWithRed:105/255.0 green:124/255.0 blue:214/255.0 alpha:1.0] forState:UIControlStateNormal];
    [buttonR addTarget:self action:@selector(doneClick) forControlEvents:UIControlEventTouchUpInside];
    UIBarButtonItem *right = [[UIBarButtonItem alloc] initWithCustomView:buttonR];
...
-(void)remove{
    ...
    [UIView animateWithDuration:0.3 delay:0.0 options:UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        _showView.frame = CGRectMake(0, 1000, [UIScreen mainScreen].bounds.size.width,  [UIScreen mainScreen].bounds.size.height);
